...
const ApplicationViewer = () => {
  ...
  const fetchApplications = async () => {
    try {
      ...
      // tuỳ API của bạn, nếu data thật nằm ở response.data.data thì đổi lại
  ...
  const handleOpenChatFromProfile = (user) => {
    setChatUser(user); // mở ChatWindow với user này
  };
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        {selectedApplicant && (
          <ApplicantProfilePreview
            ...
            onOpenChat={handleOpenChatFromProfile} // 👈 rất quan trọng
          />
        )}
      </div>

      {/* Chat window – luôn nằm ngoài modal */}
      {chatUser && (
        <ChatWindow otherUser={chatUser} onClose={handleCloseChat} />
      )}
    </DashboardLayout>
  );
};
